...
@Slf4j
@AllArgsConstructor
public class Sap285ApiWebImpl {
    final String prefixPath = "api/sap-interim/v1";
	
	private final ISapCallApiService sapCallApiService ;
	
	@RequestMapping(value = prefixPath +"/285",method = RequestMethod.POST, produces = { "application/json" })
	public ResponseEntity<String> handleTransactionTypeSaleNormal(@RequestBody String jsonRequest) {
		...
		Sap285ResBean res = null ;
		Gson gson = new Gson();
		Sap285ReqBean input = new Sap285ReqBean();
		try {
			 input = gson.fromJson(jsonRequest, Sap285ReqBean.class);
			 
			 res = sapCallApiService.call285Api(input);
