...
@Slf4j
@AllArgsConstructor
public class SapHandleTransactionTypeCompensationDepositWebImpl {
	final String prefixPath = "api/sap-handle-transaction-type/v1";

	private final ISapHandleTransactionTypeCompensationDepositService sapHandleTransactionTypeCompensationDepositService;

	@RequestMapping(value = prefixPath + "/compensation-deposit", method = RequestMethod.POST, produces = {
			"application/json" })
	public ResponseEntity<String> handleTransactionTypeCompensationDeposit(@RequestBody String jsonRequest) {
		...
		try {
			...
			DtSapTransaction dtSapTransaction = sapHandleTransactionTypeCompensationDepositService
					.queryDtSapTransactionById(input.getSapTranId());

			// step 1 query workflow
			DtSapTransactionType dtSapTransactionType = sapHandleTransactionTypeCompensationDepositService
...
//		   
		   ...
		   //1. sale order 
			if (BeanUtil.isNotEmpty(dtSapTransactionType.getSaleOrder())
				...
				// insert table
				dtSapOrderHeader = sapHandleTransactionTypeCompensationDepositService
						.queryInfoAndInsertSaleorderCompensationDeposit(dtSapTransaction);
			...
			if (BeanUtil.isNotEmpty(dtSapTransactionType.getPostTransaction())
				...
				dtSapPostHeader = sapHandleTransactionTypeCompensationDepositService
...
//		   
			...
			if (dtSapOrderHeader != null) {
				// call service post transaction to send input to sap and keep response from sap
				DtSapOrderHeader dtSapOrderHeaderApi = sapHandleTransactionTypeCompensationDepositService
				...
				sapHandleTransactionTypeCompensationDepositService.updateDtSapOrderHeader(dtSapOrderHeader);
			}

			// call service post transaction to send input to sap and keep response from sap
			if (dtSapPostHeader != null) {
				// call service post transaction to send input to sap and keep response from sap
				DtSapPostHeader dtSapPostHeaderApi = sapHandleTransactionTypeCompensationDepositService
				...
				// update status dtSapPostHeader
				sapHandleTransactionTypeCompensationDepositService.updateDtSapPostHeader(dtSapPostHeader);
			}
...
//		   
			...
			sapHandleTransactionTypeCompensationDepositService.updateDtSapTransactionAfterWorkflow(dtSapTransaction);
