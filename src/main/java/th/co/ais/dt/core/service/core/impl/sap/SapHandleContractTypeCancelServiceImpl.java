...
@Slf4j
@AllArgsConstructor
public class SapHandleContractTypeCancelServiceImpl implements ISapHandleContractTypeCancelService {
	...
	public DtSapUniversalHeader queryInfoAndInsertUniversal(DtSapContractTrans dtSapContractTrans) {
		try {
			// query
			List<QueryContractUniversalBean> listQueryUniversal = queryContractCancelUniversal(dtSapContractTrans.getCompany(), dtSapContractTrans.getDocNo());
			...
			DtSapContractTransLog saleLog = dtSapContractTransLogDao.querySapContractTransLog(dtSapUniversalHeader.getReferenceHDKey2());
			String assignment = "";
			if(saleLog != null) {
	...
	private List<QueryContractUniversalBean> queryContractCancelUniversal(String company, String docNo){
		// query from postgres ,
		//return dtSapContractTypeDao.queryContractUniversalCn(company, docNo);
		
		// query from on prem
		return queryContractCancelUniversalOnPrem(company, docNo);
	}
	
	private List<QueryContractUniversalBean> queryContractCancelUniversalOnPrem(String company, String docNo) {
...
//		String out = c.HttpClient(properties, "http://localhost:8080/DTWS/api/saleout/v1/query-contract-cancel-universal", gson.toJson(in), "POST", null) ;
		String out = c.HttpClient(properties, dTConfig.getUrl().getDtws() + "/DTWS/api/saleout/v1/query-contract-cancel-universal", gson.toJson(in), "POST", null) ;
